...
| 第二章 · 进入项目组 | LangChain —— LCEL 管道、提示词模板、输出解析器、对话记忆、链路由 |
| 第三章 · 黑糖资料室 | RAG —— 向量嵌入、向量库、检索增强管线 |
| 第四章 · 快速执行术 | Agent 控制 —— 工具调用、结构化工具、记忆、人工介入 |
| 第五章 · 工具开发 | Harness 工程 —— 上下文窗口、错误韧性、可观测性、插件架构 |
| 第六章 · 协作评审 | 多 Agent —— 消息总线、Supervisor、辩论模式 |
