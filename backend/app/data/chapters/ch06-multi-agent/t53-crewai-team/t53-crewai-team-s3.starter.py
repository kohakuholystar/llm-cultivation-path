...
def ask_llm(llm: ChatOpenAI, agent: Agent, task: Task, context: str) -> str:
    ...
    # TODO: 拼出 system 与 user 两条消息并调用 llm.invoke
    # 提示: system = f"你是{agent.role}。目标:{agent.goal}。背景:{agent.backstory}。请始终用中文作答。";
    #       user = f"军令:{task.description}\n验收标准:{task.expected_output}",context 非空时追加 "\n前置产出(供参考):{context}";
    #       reply = llm.invoke([SystemMessage(content=system), HumanMessage(content=user)]),return reply.content
    raise NotImplementedError("ask_llm 尚未实现:请按 TODO 提示拼出消息并调用 llm.invoke")
...
class Crew:
    ...
    def kickoff(self) -> list[str]:
        ...
        # TODO: 补全顺序执行循环:找执行者、打印进度、执行收集、更新上下文
        # 提示: results = [];context = "";for i, task in enumerate(self.tasks, 1):
        #       agent = self._agent_for(task.role);print(f"军令 {i}/{len(self.tasks)} → {task.role}:{task.description}");
        #       output = self._run_one(agent, task, context);results.append(output);context = output;print(f"  产出:{output[:40]}...")
        raise NotImplementedError("kickoff 尚未实现:请按 TODO 提示补全顺序执行循环")
