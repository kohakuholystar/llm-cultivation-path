...
def route_task(task: AgentTask) -> str:
    ...
    # TODO: 遍历 SKILL_ROUTES,关键词命中即返回对应仙官,循环结束兜底后端仙官
...
def specialist_work(task: AgentTask) -> TaskResult:
    ...
    # 提示: if task.assignee == "前端仙官": data = f"已产出页面骨架: {task.title}"
    #       elif task.assignee == "测试仙官": data = f"已编写测试用例 {len(task.title)} 条,全部通过"
    #       elif task.assignee == "文档仙官": data = f"文档已更新: {task.title}"
