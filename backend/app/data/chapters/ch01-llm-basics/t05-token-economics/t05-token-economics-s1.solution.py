...
def main() -> None:
    ...
    ids = meter.encoding.encode("灵讯通")
    print(f"[灵讯通] '灵讯通' 被切成 {len(ids)} 个 token,id = {ids}")
    print("[灵讯通] 字符数 vs token 数:")
    ...
    print(f"[灵讯通] 本组消息共 {total} tokens (每条另含 {CHAT_OVERHEAD} tokens 包装开销)")
