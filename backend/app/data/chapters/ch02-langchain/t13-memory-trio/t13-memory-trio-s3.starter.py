...
def count_tokens(messages) -> int:
    ...
    # TODO: 给记忆账本计价:有编码器按编码数,离线按字符近似
    # 提示: ENCODING.encode(m.content) 得单条消息 token 数,每条另有 +4 结构开销
    #       ENCODING 为 None 时用 len(m.content) + 4 近似
    raise NotImplementedError("count_tokens 尚未实现:请按 TODO 提示完成 token 估算")


def run_dialogue(advisor, script):
    """把同一份剧本灌给顾问,返回每轮过后记忆账本的 token 数列表。"""
    # TODO: 逐轮灌剧本,记录每轮过后的 token 数并返回列表
    # 提示: 每轮先 advisor.chat(line),再用 count_tokens 统计当前账本:
    #       advisor.memory.load_memory_variables({})["history"] 即账本里的消息列表
    raise NotImplementedError("run_dialogue 尚未实现:请按 TODO 提示完成逐轮记账")
