...
class ForgeAdvisor:
    ...
    def __init__(self, model, memory=None):
        self.model = model
        # TODO: 给顾问配一本全量记忆账本
        # 提示: ConversationBufferMemory(memory_key="history", return_messages=True)
    ...
    def chat(self, user_input: str) -> str:
        ...
        # 提示: history = self.memory.load_memory_variables({})["history"]  # 翻账
        #       reply = self.model.invoke([*history, HumanMessage(content=user_input)]).content  # 连账带话发给模型
        #       self.memory.save_context({"input": user_input}, {"output": reply})  # 答完立刻记账
