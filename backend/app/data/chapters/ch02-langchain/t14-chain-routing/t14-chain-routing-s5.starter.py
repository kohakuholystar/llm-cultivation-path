...
def workbench(router, request: str, history: list) -> None:
    ...
    # TODO: 接待一位客人:打印、分流、渲染,闲聊登记入问答录
    # 提示: print(f"客人:「{request}」")
    #       intent, reply = router.invoke({"request": request, "history": list(history)})  # history 传拷贝:模型看到接待前的快照
    #       reply 是 SwordOrder 时格式化成 f"铸剑单 · {reply.name}({reply.material},锋芒{reply.sharpness}):「{reply.inscription}」"
