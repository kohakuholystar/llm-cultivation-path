...
def forge_with_retry(theme: str) -> SwordOrder:
    ...
    prompt = build_prompt()
    llm = get_llm()
    # TODO: 创建 retry_parser = RetryWithErrorOutputParser.from_llm(
    #           parser=parser, llm=llm, max_retries=2)
    # 提示: 内层仍是 parser,失败时用 llm 按「原输出 + 报错」重写后再试
    ...
    # TODO: 用 retry_parser.parse_with_prompt(completion, prompt_value) 解析并返回结果
    # 提示: 这里必须用 parse_with_prompt 而不是 parse——基类的 parse 没有重试逻辑,
    #       坏输出照样抛 OutputParserException;parse_with_prompt 才会把错误喂回模型
    raise NotImplementedError("forge_with_retry 尚未实现:请按 TODO 提示完成重试解析")
