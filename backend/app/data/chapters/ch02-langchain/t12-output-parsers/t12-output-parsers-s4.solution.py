...
class ForgeCardParser(BaseOutputParser):
    ...
    def parse(self, text: str) -> SwordOrder:
        ...
        for line in text.strip().splitlines():
            if ":" not in line:
                continue  # 跳过【铸剑帖】标题行等非键值行
            key, _, value = line.partition(":")  # 只按第一个冒号切,铭文里含冒号也安全
        ...
        if missing:
            ...
            raise OutputParserException(f"铸剑帖缺字段: {missing}")
        try:
            data["sharpness"] = int(data["sharpness"])
        except ValueError as exc:
            raise OutputParserException(f"锋芒值不是整数: {data['sharpness']}") from exc
        return SwordOrder(**data)  # Pydantic 再做一次契约校验(如锋芒 1-100)

    def get_format_instructions(self) -> str:
        return (
            "请严格按以下格式输出铸剑帖,每行一个字段,不要输出任何其他内容:\n"
            "剑名:<剑名>\n主材:<主材>\n锋芒:<1-100的整数>\n铭文:<铭文>"
        )
