"""百宝囊 · 第2关: 请出执行官 AgentExecutor
新增宝器「度量衡」, 用 AgentExecutor.invoke 替换 t31 手写的 while 循环。
"""
import os
import sys
...
def duliangheng(query: str) -> str:
    ...
    # TODO: 解析「数值 源单位 目标单位」, 以米为桥换算, 异常返回「换算失败」文本
    # 提示: try:
    ...
    raise NotImplementedError("t32-s2-duliangheng 尚未实现:请按 TODO 提示完成以米为桥的换算")
...
MOCK_SCRIPT = [
    ...
    "Thought: 我现在知道最终答案了\nFinal Answer: 3*(4+5)=27;杭州晴 26°C;3 里 = 1500 米",
]


def build_executor(tools: list, script: list) -> AgentExecutor:
    """组装执行官: agent(决策) + tools(手脚) -> AgentExecutor(运行时)。"""

    agent = create_react_agent(build_llm(script), tools, REACT_PROMPT)
    # TODO: 返回 AgentExecutor(agent, tools) 作为运行时
    # 提示: return AgentExecutor(agent=agent, tools=tools)
    raise NotImplementedError("t32-s2-executor 尚未实现:请按 TODO 提示返回 AgentExecutor")
...
def main() -> None:
    ...
    print("== 百宝囊 · 三件宝器已上架 ==\n")
    for q in ["3*(4+5) 等于多少?", "杭州天气如何?另外 3 里是多少米?"]:
        # TODO: invoke 并打印问/答
        # 提示: ans = executor.invoke({"input": q})
        #       print(f"问: {q}\n答: {ans['output']}\n")
        raise NotImplementedError("t32-s2-invoke 尚未实现:请按 TODO 提示完成问答循环")
