...
def shenji_suanpan(expression: str) -> str:
    ...
    # TODO: 用受限 eval 计算表达式, 异常返回「算盘打不出:」文本
    # 提示: try: return str(eval(expression, {"__builtins__": {}}, {}))
    #       except Exception as exc: return f"算盘打不出: {exc}"
    raise NotImplementedError("t32-s1-suanpan 尚未实现:请按 TODO 提示完成受限 eval 计算")
...
def main() -> None:
    ...
    # TODO: 三件套合体并 invoke, 打印答复
    # 提示: agent = create_react_agent(build_llm(MOCK_SCRIPT), tools, REACT_PROMPT)
    #       executor = AgentExecutor(agent=agent, tools=tools)
    #       ans = executor.invoke({"input": "3*(4+5) 等于多少?"})
    #       print("百宝囊答复:", ans["output"])
    raise NotImplementedError("t32-s1-agent 尚未实现:请按 TODO 提示组装 Agent 并跑一次问答")
