...
def build_executor(tools: list, script: list) -> AgentExecutor:
    ...
    # TODO: 集齐工程化全家桶: 熔断 + 自愈 + 直播 + 留底
    # 提示: return AgentExecutor(
    #           agent=agent, tools=tools,
    #           max_iterations=5, handle_parsing_errors=True,
    #           verbose=True, return_intermediate_steps=True)
    raise NotImplementedError("t32-s5-verbose 尚未实现:请按 TODO 提示开启 verbose 与轨迹留底")


def print_trajectory(result: dict) -> None:
    """回放结构化轨迹: 逐步列出 (宝器, 输入, 回报)。"""

    # TODO: 读 intermediate_steps 逐条回放: 宝器名、输入与回报
    # 提示: steps = result.get("intermediate_steps", [])
    #       for i, (action, observation) in enumerate(steps, 1):
    #           print(f"  第{i}步 宝器[{action.tool}] 输入={action.tool_input!r} 回报={observation}")
    raise NotImplementedError("t32-s5-trajectory 尚未实现:请按 TODO 提示完成轨迹回放")
