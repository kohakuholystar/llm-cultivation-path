...
def build_executor(tools: list, script: list) -> AgentExecutor:
    ...
    # TODO: 给 AgentExecutor 加自我疗愈: 解析出错时把错误喂回模型重试
    # 提示: return AgentExecutor(
    #           agent=agent, tools=tools,
    #           max_iterations=5, handle_parsing_errors=True)
    raise NotImplementedError("t32-s4-heal 尚未实现:请按 TODO 提示开启 handle_parsing_errors")
...
def main() -> None:
    ...
    if MOCK:
        ...
        # TODO: 接住未开恢复时的崩溃, 打印 ValueError 开头一段
        # 提示: try: bare.invoke({"input": "演示"})
        #       except ValueError as exc: print("未开恢复时崩溃:", str(exc).split(".")[0])
        raise NotImplementedError("t32-s4-bare 尚未实现:请按 TODO 提示完成对照组崩溃演练")
