...
script = [
    ...
    {"content": "筑基丹要以九转灵草为引,文火炼足四十九日;配方详见典籍。"},
    {"content": "", "tool_calls": [{"name": "search_knowledge", "args": {"query": "雷劫丹药"}, "id": "call_2"}]},
    {"content": "渡劫丹药在丹房二层,需避雷木作辅料,详情可查典籍。"},
    {"content": "窗口里还剩最近 6 条消息,更早的已经压进摘要了。"},
    {"content": "最早那句「筑基丹怎么炼?」已被压缩进摘要,我依然记得。"},
]
...
def run_turn(user_text: str) -> None:
    ...
    if reply.tool_calls:
        ...
        reply = llm.invoke(messages)
    print("渡劫台:", reply.content)
    memory.add("assistant", reply.content)


def main() -> None:
    for q in ["筑基丹怎么炼?", "那雷劫丹药在哪炼?", "现在窗口还剩几条?", "最早问的那句还记得吗?"]:
        print(f"\n—— 道友问: {q}")
        run_turn(q)
    print("\n[检查] 剧本全部消耗:", not script)
    print("[检查] 摘要内容:", memory.summary)
    print("[检查] 摘要含最早提问:", "筑基丹怎么炼?" in memory.summary)
    print("[检查] 窗口剩余:", len(memory.messages))
    assert not script and "筑基丹怎么炼?" in memory.summary
